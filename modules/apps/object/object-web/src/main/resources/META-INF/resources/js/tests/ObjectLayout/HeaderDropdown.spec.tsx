...
function mockLayoutContextWithBoxes(boxTypes: Array<string> = []) {
	const objectLayoutBoxes = boxTypes.map((type) => ({type}));
...
describe('HeaderDropdown component', () => {
	...
	it('disables "add-categorization" if it already exists in a tab', () => {
		mockLayoutContextWithBoxes(['categorization']);
	...
	it('disables "add-seo" if it already exists in a tab', () => {
		mockLayoutContextWithBoxes(['seo']);
		renderHeaderDropdown();
		openDropdown();
